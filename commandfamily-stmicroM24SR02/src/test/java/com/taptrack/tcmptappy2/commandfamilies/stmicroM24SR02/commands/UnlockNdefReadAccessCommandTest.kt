...
class UnlockNdefReadAccessCommandTest {
    @Test
    @Throws(Exception::class)
    fun testGetCommandCode() {
        val command = UnlockNdefReadAccessCommand()
        Assert.assertEquals(command.commandCode, 0x06.toByte())
    }
    ...
    fun testPayload() {
        ...
        val command = UnlockNdefReadAccessCommand(payload)
    ...
    fun testLogicalParameters() {
        ...
        val command = UnlockNdefReadAccessCommand(timeout, password)
        Assert.assertArrayEquals(command.payload, payload)
        Assert.assertEquals(command.timeout, timeout)
