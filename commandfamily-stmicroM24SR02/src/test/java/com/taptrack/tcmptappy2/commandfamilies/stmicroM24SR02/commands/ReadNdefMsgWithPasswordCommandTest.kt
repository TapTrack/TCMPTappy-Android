...
class ReadNdefMsgWithPasswordCommandTest {
    @Test
    @Throws(Exception::class)
    fun testGetCommandCode() {
        val command = ReadNdefMsgWithPasswordCommand()
        Assert.assertEquals(command.commandCode, 0x08.toByte())
    }
    ...
    fun testPayload() {
        ...
        val command = ReadNdefMsgWithPasswordCommand(payload)
    ...
    fun testLogicalParameters() {
        ...
        val command = ReadNdefMsgWithPasswordCommand(timeout, password)
        Assert.assertArrayEquals(command.payload, payload)
        Assert.assertEquals(command.timeout, timeout)
