...
class UnlockNdefWriteAccessCommandTest {
    @Test
    @Throws(Exception::class)
    fun testGetCommandCode() {
        val command = UnlockNdefWriteAccessCommand()
        Assert.assertEquals(command.commandCode, 0x05.toByte())
    }
    ...
    fun testLogicalParameters() {
        ...
        val command = UnlockNdefWriteAccessCommand(timeout, password)
        Assert.assertArrayEquals(command.payload, payload)
        Assert.assertEquals(command.timeout, timeout)
    ...
    fun testError(){
        ...
        Assert.assertThrows("Payload length isnt 17", MalformedPayloadException::class.java){
            val command = UnlockNdefWriteAccessCommand(payload)
        }
    }
}